...
| 7 | See the handoff before you start. | Pass |
...
| 1 | Export | Pass |
| 3 | How it works | Pass |
| 5 | Prepare, confirm, and hand over. | Pass |
...
| 11 | Encrypt packets in the browser before saving them in IndexedDB. | Pass |
...
| 12 | Sample data uses the separate `demo:closeout-kit-v1` IndexedDB database. | Pass |
| 8 | It never reads or writes `closeout-kit-v1`. | Pass |
| 15 | `npm test` runs unit tests and creates a production build. | Pass |
| 10 | It also checks Chromium flows, accessibility, mobile layout, exports, and offline reload. | Pass |
