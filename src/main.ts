...
function welcomeView(): string {
  ...
  <section class="preview-section" aria-labelledby="preview-heading"><div><span class="eyebrow">Filled packet preview</span><h2 id="preview-heading">Preview a filled client packet.</h2><p>The sample shows assets, owners, access tasks, support dates, and acknowledgement in one packet.</p><a href="/demo">Open the sample packet</a></div><div class="preview-sheet"><span>Northstar Arts website</span><strong>3 assets</strong><strong>2 access tasks</strong><strong>Support through 27 September</strong><small>Sample data</small></div></section>
...
function stepActions(backLabel: string | null, nextLabel: string): string {
  return `<div class="step-actions">${backLabel ? `<button class="quiet" data-action="back">${backLabel}</button>` : '<span></span>'}<button class="primary" data-action="next">${nextLabel} ${icon('arrow')}</button></div>`;
}
...
function engagementStage(p: Packet): string {
  return `${stageHeader('Stage 01 · Engagement', 'Describe the finished project.', 'Add the details your client will see at the top of the packet.')}
    ...
    ${textArea('Outcome summary', 'summary', p.summary, 'Describe what was delivered and the operator outcome. Do not include secrets.')}${stepActions(null, 'Review assets')}`;
}
...
function supportStage(p: Packet): string {
  ...
  ${textArea('Included during the window', 'support.included', p.support.included, 'Example: production defects in delivered work, access clarification, one operator walkthrough.')}${textArea('Outside the window / new work', 'support.excluded', p.support.excluded, 'Example: feature requests, vendor outages, content changes, and third-party subscription costs.')}${stepActions('Review access tasks', 'Collect client receipt')}`;
}
...
function acknowledgementStage(p: Packet): string {
  ...
  <details class="history"><summary>Packet history (${p.history.length})</summary><ol>${p.history.map((event) => `<li><time datetime="${event.at}">${new Date(event.at).toLocaleString()}</time>${value(event.label)}</li>`).join('')}</ol></details>${stepActions('Set support dates', 'Review exports')}`;
}
...
function exportStage(p: Packet): string {
  ...
  ${demoMode ? '' : '<div class="danger-zone"><div><strong>Delete this packet</strong><p>Download a backup first. Deletion cannot be undone.</p></div><button class="danger-button" data-action="delete-packet">Delete packet</button></div>'}${stepActions('Review acknowledgement', 'Return to engagement')}`;
}
