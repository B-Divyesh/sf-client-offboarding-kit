...
export function buildPacketHtml(packet: Packet): string {
  ...
  const assets = packet.assets.length
    ? packet.assets.map((asset) => `<tr><td><strong>${escapeHtml(asset.name)}</strong><small>${escapeHtml(asset.kind)}</small></td><td>${asset.url ? `<a href="${escapeHtml(asset.url)}">Open original service</a>` : '—'}</td><td>${escapeHtml(asset.currentOwner)} → ${escapeHtml(asset.destinationOwner)}</td><td>${multiline(asset.note) || '—'}</td></tr>`).join('')
...
<section><h2>Access tasks</h2><ul>${actions}</ul></section>
...
<p class="notice">Generated in this browser with Closeout Kit. This packet contains links and ownership details, not passwords, API keys, or private keys. Verify access in each original service.</p></main></body></html>`;
}

export function buildAcknowledgementHtml(packet: Packet): string {
  const receiptName = `${filenameFor(packet, 'acknowledgement.json')}`;
  const packetData = JSON.stringify({ packetId: packet.id, projectName: packet.projectName, receiptName }).replace(/</g, '\\u003c');
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Acknowledge ${escapeHtml(packet.projectName)}</title><style>:root{color-scheme:light}*{box-sizing:border-box}body{margin:0;background:#e9e5da;color:#142023;font:17px/1.55 system-ui,sans-serif}main{max-width:720px;margin:auto;min-height:100vh;background:#f7f4eb;padding:clamp(24px,7vw,64px);border-top:10px solid #d9784a}h1{font:700 clamp(34px,7vw,52px)/1.05 Georgia,serif}label{display:flex;gap:12px;align-items:flex-start;margin:16px 0}input[type=checkbox]{width:22px;height:22px;flex:0 0 auto}input[type=text],input[type=date]{width:100%;min-height:48px;padding:10px;border:2px solid #526365;background:white;font:inherit}.field{display:block;font-weight:700}.field input{margin-top:6px}button{min-height:48px;padding:12px 18px;border:0;background:#b65b35;color:white;font:700 16px system-ui;cursor:pointer}:focus-visible{outline:3px solid #0b6266;outline-offset:3px}.note{padding:16px;border-left:4px solid #b65b35;background:#efe9dc}.facts{border-block:1px solid #c7c0b2;padding:12px 0}#status{font-weight:700}</style></head><body><main><p>Closeout Kit acknowledgement</p><h1>Confirm receipt of ${escapeHtml(packet.projectName)}</h1><p>${escapeHtml(packet.clientName)} can return a small receipt file to ${escapeHtml(packet.preparedBy)}. This is acknowledgement, not a legal e-signature.</p><div class="facts"><strong>${packet.assets.length} assets</strong> · <strong>${packet.actions.length} access tasks</strong> · support ends <strong>${niceDate(packet.support.ends)}</strong></div><form id="receipt"><label><input name="received" type="checkbox" required> I received the listed assets and links.</label><label><input name="ownership" type="checkbox" required> I understand the documented ownership state.</label><label><input name="noSecrets" type="checkbox" required> I understand credentials are shared separately.</label><label class="field">Your name<input name="signer" type="text" required autocomplete="name"></label><label class="field">Role<input name="role" type="text"></label><label class="field">Date<input name="signedAt" type="date" required></label><button type="submit">Download acknowledgement receipt</button><p id="status" aria-live="polite"></p></form><p class="note">Return the downloaded JSON file to the packet preparer. It contains only this acknowledgement and your typed details.</p></main><script>const packet=${packetData};document.querySelector('#receipt').addEventListener('submit',event=>{event.preventDefault();const data=new FormData(event.currentTarget);const receipt={format:'closeout-kit-acknowledgement',version:1,packetId:packet.packetId,projectName:packet.projectName,received:true,ownership:true,noSecrets:true,signer:String(data.get('signer')),role:String(data.get('role')),signedAt:String(data.get('signedAt'))};const url=URL.createObjectURL(new Blob([JSON.stringify(receipt,null,2)],{type:'application/json'}));const link=document.createElement('a');link.href=url;link.download=packet.receiptName;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);document.querySelector('#status').textContent='Receipt downloaded. Return it to the packet preparer.'});</script></body></html>`;
}
